...
impl SniffType {
    ...
    pub fn defaults() -> Vec<Self> {
        vec![Self::Tls, Self::Http, Self::Quic, Self::Ssh, Self::BitTorrent]
    }
}
...
fn try_quic(buf: &[u8]) -> Option<SniffResult> {
    ...
    pos += 1;
    if dcid_len == 0 || dcid_len > 20 { return None; }
    ...
    if encrypted_payload.is_empty() { return None; }
    ...
    match prefix {
        ...
        3 => {
            ...
            let v = u64::from_be_bytes([
                first & 0x3F, buf[pos+1], buf[pos+2], buf[pos+3],
...
fn decrypt_quic_initial(
    ...
    if payload.len() < pn_len { return None; }
    ...
    if payload.len() < ciphertext_start + 16 { return None; }
...
fn aes128_ecb_block(key: &[u8], block: &[u8]) -> Option<[u8; 16]> {
    if key.len() != 16 || block.len() < 16 { return None; }
...
fn aes128_gcm_decrypt(
    ...
    if key.len() != 16 || nonce.len() != 12 || tag.len() != 16 { return None; }
...
fn aes128_encrypt_block(state: &mut [u8; 16], round_keys: &[[u8; 16]; 11]) {
    ...
    for (round, rk) in round_keys[1..].iter().enumerate().map(|(i, rk)| (i + 1, rk)) {
        // SubBytes
...
fn extract_sni_from_quic_crypto(data: &[u8]) -> Option<SniffResult> {
    ...
    while pos < data.len() {
        ...
        match frame_type {
            ...
            0x06 => {
                // CRYPTO frame
                let (_, vl) = read_varint(data, pos)?; pos += vl; // offset
                let (flen, vl2) = read_varint(data, pos)?; pos += vl2;
